...
export interface IPostRepository<T> {
  save(postId: string, data: T): Promise<void>;
}

class FacebookRepository {
  constructor() {}

  getStoragePaths(postId: string): PostStoragePaths {
  ...
  async ensurePostDirs(postId: string): Promise<PostStoragePaths> {
  ...
  getPostStoragePaths(postId: string): PostStoragePaths {
    const postDir = path.join(FILE_PATHS.facebook.scrapedDataDir, postId);
    const mediaDir = path.join(postDir, "media");
    return {
      postDir,
