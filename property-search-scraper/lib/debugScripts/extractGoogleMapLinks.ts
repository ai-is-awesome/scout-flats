const tests = [
  `Female Flatmate Needed (Vegetarian)
Looking for a female replacement in a 3BHK fully furnished apartment. 
Location: Meru Aadhi apartment, silver spring layout, Kundanahalli gate
https://maps.app.goo.gl/ujQjJxSM6dcjWU5z5?g_st=ic
Room details-
* Full size bedroom
* Attached washroom with geyser
* Queen sized bed
* Study table
* Huge closet with mirror
* Plenty of storage space
Property includes lift, 4 wheeler parking space, 24*7 power backup, 24*7 security with mygate
The flat comes with sofa, coffee table, TV, dining table, washing machine, microwave, fridge and a fully set up kitchen.
Location perks :- 
Bus stop, Cult Gym, Westside/Zudio, Star Extra mall - all just 500m away.
Costs(per month, per person) :-
Rent - ₹16,750
Maintenance- ₹1500
Security deposit - ₹33,333 (one-time payment)
One time setup cost - ₹15500
Maid - ₹1000 (3000/3)
Cook optional
Electricity - Gruha Jyoti Scheme enabled, 100 units free every month
Wi-Fi, electricity, gas(split among 3 depending on the usage)
DM for more details!! See less
`,
];
